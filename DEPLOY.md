...
echo $?          # 0 — успех (в т.ч. частичный, см. ниже); 3 — документ; 4 — пакет; 5 — модель
# Отказ модели больше не всегда даёт 5: если детерминированный слой успел
# что-то найти, ядро отдаёт его находки с кодом 0 и предупреждением
# MODEL_UNAVAILABLE_PARTIAL. Пятёрка остаётся, когда показывать нечего.
...
Каталог пакетов правил приложение ищет у себя; положим пакеты туда, куда оно
смотрит, и **зарегистрируем каждый в базе**.

⚠️ **Файлы на диске каталог не наполняют.** Приложение показывает только
записи из `review_pack_references`: пакет, выложенный без регистрации,
невидим — ни в выборе профиля, ни в блоке состава. Так уже случилось
с профилем `generic`.

Ключ и версия обязаны совпадать с манифестом пакета, иначе приложение
забракует результат ядра как несоответствующий заданию. Поэтому регистрация
идёт скриптом, который читает идентичность из манифеста, а не руками:

```bash
for PACK in mts-net/0.2 generic/1.0; do
  python3 scripts/register-review-pack.py \
    --pack /opt/docreview/data/review-packs/$PACK \
    --packs-root /opt/docreview/data/review-packs \
...
- **PDF поддерживается** с 5 сентября (`pdf_text.py` на `pypdf`, зависимость
  в `requirements-core.txt`). У формата есть цена, и результат о ней
  предупреждает: PDF не хранит разметку таблиц, поэтому проверки «по ячейкам»
  на нём слабее, чем на `.docx`. Скан без текстового слоя отбивается
  с объяснением, а не анализируется впустую.
