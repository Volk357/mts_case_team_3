...
| `finding.confidence` | откалиброванная доля попаданий по слою и важности; в ранжировании НЕ участвует — `docs/confidence-calibration.md` |
...
по важность × согласие проходов. Детерминированные типы берутся ТОЛЬКО из формального
слоя (модельные дубли этих типов отбрасываются).

**Порядок в `findings` значимый:** severity, затем важность × согласие проходов,
