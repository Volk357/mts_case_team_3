...
- Рост слоя модели после доработок 4 сентября (с 68% до 82–84%) НЕ объяснён:
  свежие и старые прогоны различаются одновременно промптом, JSON-схемой и
  версией бюджетирования, контрольного прогона не делалось. Гипотеза — из промпта
  ушли шесть типов, переведённых в детерминированный слой, — остаётся гипотезой.
