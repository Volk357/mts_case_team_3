...
## Проверить наши цифры за минуту, без модели и без доступа к контуру

Всё нужное лежит в этом репозитории. Модель не понадобится: прогоны слоя
модели закоммичены как артефакты, а эталон порождается кодом.

```bash
# 1. Эталон воспроизводится побайтово — генератор, а не набор файлов
python3 generate.py --out /tmp/synth && diff -rq data/synth /tmp/synth   # отличий нет

# 2. Полнота на закоммиченных артефактах прогона
python3 score.py --batch data/synth data/ab_taxonomy/outputs --suffix _newtax_full2
#   полнота все   по месту 89% (58/65)   и тип 78% (51/65)
#   слой детерм.  по месту 96% (26/27)

# 3. Слой правил считается прямо здесь, без сети
python3 check_formal.py --doc data/synth/synth_1.txt

# 4. Тесты ядра
for t in test_*.py; do python3 "$t"; done
```

Заголовочные 88% / 77% — меньшее из четырёх наблюдений на прогонах `results/enum`
и `results/enum2`; команда выше даёт 89% / 78% на артефактах A/B-эксперимента.
Разница в один-два дефекта — это разброс между прогонами при `temperature 0`,
и он назван честно, а не сглажен.
...
- [Совместная отладка Product Application и Analysis Core](docs/core-collaborative-debugging.md);
