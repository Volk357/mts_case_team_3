...
Профиль `mock` запускает PostgreSQL, миграции, demo seed, API, детерминированный
worker и frontend. Он не требует модели и подходит для проверки полного
...
Профиль `real` использует те же API, базу и хранилища, но запускает настоящее
Analysis Core только после успешного `model-preflight`:

```powershell
docker compose --profile real up --build --wait
```

Профили нельзя включать одновременно: иначе два worker будут конкурировать за
одну очередь. Frontend доступен на `http://127.0.0.1:8080`; порт меняется через
`DOCREVIEW_HTTP_PORT`. API наружу отдельно не публикуется и доступен браузеру
через nginx по пути `/api`.

Redis не используется: задания атомарно забираются worker непосредственно из
PostgreSQL, который уже обеспечивает блокировку и восстановление очереди.
Долгоживущие сервисы перезапускаются при сбое; миграции, seed и проверки остаются
одноразовыми. Лимиты CPU/RAM относятся только к приложению — модель работает на
внешнем GPU-сервере и ими не ограничивается.
