...
# Уверенность — ОТКАЛИБРОВАННАЯ доля попаданий, а не вес важности.
#
# Пункт 8 аудита: «0.95–0.99 выглядит как вероятность, но вычисляется
# эвристически из severity. Такой показатель вводит пользователя
# в заблуждение». Претензия подтвердилась замером и оказалась хуже,
# чем звучала: показатель был не просто неоткалиброван, а местами
# инвертирован — заявленные 0.95 у модели давали 20% попаданий,
# а 0.85 — 70%.
#
# Замер: 5 synth-документов, 107 выданных замечаний, сопоставление
# с эталоном той же функцией, что считает полноту (score.match).
#
#   слой правил           заявлено 0.95 → фактически 100% (26/26)
#   модель, high          заявлено 0.85 → фактически  51% (21/41)
#   модель, medium        заявлено 0.70 → фактически  19% (6/32)
#   модель, low           заявлено 0.55 → фактически  60% (3/5)
#   модель, clarification заявлено 0.50 → фактически  33% (1/3)
#
# Значения ниже взяты по замеру там, где наблюдений достаточно (high, medium),
# и КОНСЕРВАТИВНО там, где их почти нет: low, clarification и critical
# получают оценку medium. Пять наблюдений — не основание объявить 60%,
# и завышать по ним нельзя. Это названо здесь, а не спрятано.
#
# Слой правил остаётся 0.95, хотя замер дал 26/26. Единица означала бы
# «ошибок не бывает», а у слоя есть известный промах (SCHEMA_TYPE_MISMATCH
# на synth_2, где соседняя мутация стёрла вторую сторону сравнения).
#
# ОГРАНИЧЕНИЕ, которое надо произносить вслух: калибровка снята на
# синтетическом эталоне. «Попадание» здесь — совпадение с дефектом,
# который внесли мы сами. На документах заказчика доля будет другой.
_CONFIDENCE_RULE_LAYER = 0.95
_SEV_BASE = {"high": 0.5, "medium": 0.2, "low": 0.2, "clarification": 0.2,
             "critical": 0.5}
...
def _confidence(finding, deterministic):
    """Доля попаданий, ожидаемая для замечания такого слоя и важности.

    Бонус за согласие проходов (`merged_count`) убран. Он выглядел
    осмысленно — «замечание, к которому сошлось несколько проходов,
    вероятнее реально», — но замер показал обратное: значения, набранные
    этим бонусом, калибруются ХУЖЕ базовых. У модели 0.95 давало 20%
    попаданий, 0.99 — 50%, тогда как базовые 0.90 давали 69%. То есть
    надбавка не добавляла достоверности, а разрушала шкалу.

    Согласие проходов при этом остаётся в РАНЖИРОВАНИИ (`run_review._priority`,
    там `merged_count` — множитель важности). Это разные вещи: «показать
    выше» и «вероятнее окажется правдой», и раньше они были склеены.
    """
    if deterministic:
        return _CONFIDENCE_RULE_LAYER
    return _SEV_BASE.get(finding.get("severity", "medium"), 0.2)
...
def _rank_union(formal_findings, llm_findings, ceiling=BUDGET):
    ...
    построению, галлюцинаций не даёт); остаток добираем
    по важность × согласие проходов — той же `run_review.apply_budget`,
    что работает внутри слоя модели, чтобы правило было одно, а не два.
    (Это НЕ поле `confidence` из выдачи: оно в ранжировании не участвует,
    см. `_confidence` и docs/confidence-calibration.md.) Если одних защищённых больше
    ...
    выдачу; порядок в ней — общий для всех: severity, затем важность ×
    согласие проходов, детерминированное вперёд при равенстве. Иначе
